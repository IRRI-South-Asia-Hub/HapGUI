hap_phe <- function(gene_infile, pheno_file, select_cri, dir){
  ...
  for (gun in genes) {
    ...
    colm<- colm[-1]
    file_path <- system.file("snpsift",paste0(gun,".csv"),
                             package = "HaploGUI")
    file.copy(from = file_path,to = paste0(getwd(),"/",gun,".csv"))
    gc<-c(gun)
    for (jun in colm) {
      dir.create(jun)
      P <-all_pheno %>% dplyr::select(starts_with(jun))
      row.names(P) <- all_pheno$X.Phenotype.
      write.csv(P ,file.path(file = "pheno.csv"),row.names = TRUE)
      haplo_P <- file.path(getwd(),jun)
      z<- haplo_P
      setwd(paste0(z))
      ...
      if (ncol(a) <=1) {
        ...
        write.csv(a,file.path((paste0(getwd())),file = "NULL.csv"))
        gcc<- "all"
        ...
        write.csv(haploy[,-c(4)],file.path(file = "Haplotype.csv"))
        
        write.csv(var_pheno,file.path( file = "Variety.csv"))
        
        write.table(genotype,file.path(file = "Flapjack.genotype"),sep="\t", quote = F,row.names = F,col.names = F)
        ...
        if(length(hp[,1]) >=2)
        {
          ...
          if( sum(aooov$effects == 0)){
            sink(file.path((paste0(snpfile,"")), "no_anova.txt"))
            ...
            sink(file.path( "duncan_pval.txt"))
            ...
            jpeg(file.path("boxplot.jpg"), height = 1200,width = 1400, res = 300)
        ...
        write.csv(haplo,file.path(file = "Haplotype.csv"))
        
        write.csv(variety,file.path( file = "Variety.csv"))
      ...
      if( gcc=="NA"){
        ...
        happi <- str_c(row.names(mean_val),
