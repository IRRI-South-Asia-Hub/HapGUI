qc_linux <- function(choice_geno,phefile,ip_dir, theme_cus) {
  ...
  system(paste0(ip_dir, "/tassel5/run_pipeline.pl -fork1 -vcf marker.vcf -homozygous -export marker -exportType Hapmap"))
  ...
  mar <- mar[, -c(2, 5:11)]
  pca1 <- read.delim2("pca.eigenvec", header = FALSE)
  ...
  mar <- mar[-1,]
  write.csv(mar, "marker.csv", row.names = FALSE)
  ...
  comb_p$sub <- factor(comb_p$sub)
  
  p1 <- ggplot(comb_p, aes(x = PC1, y = PC2, color = sub)) +
    geom_jitter()+theme_cus
  
  ggsave("pca_plot_2D.png", p1, width = 12, height = 10, dpi = 600)
