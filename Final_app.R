setwd("/media/niran/Expansion/HaploGUI_part2/")
#initial-reqs----
dir <- getwd()
ip_dir <- "softwares/"
...
ui = fluidPage(tagList(
  ...
  navbarPage(
    ...
    id = "my-page",
    title = actionLink("title", "HaploGUI"),
    tabPanel(
      "Home",icon = icon("home"),
...
server = function(input, output, session) {
  ...
  observeEvent(input$run_extract,{
    ...
    marker <- qc_linux(phefile = input$geno_extract$datapath,ip_dir = ip_dir)
  ...
  observeEvent(input$runa,{
    ...
    result <- mrMLM(fileGen = file.path(dir,"marker.csv"), 
                    ...
                    fileKin = NULL, filePS = file.path(dir,"pca.csv"),
