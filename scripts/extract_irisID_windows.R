extract_irisID_windows <- function(trait, infile, perc, dir, ip_dir){
  ...
  file.copy(from = "marker.bim",to = paste0(data_dir,infam,".bim"))
  file.copy(from = "marker.bed",to = paste0(data_dir,infam,".bed"))
  ...
  dat <- fread("et_marker.afreq", select = c(3:4))
  colnames(dat) <- c("Ref","Alt")
  write.table(dat, file = allele_file, row.names = F, col.names = T, 
              sep = "\t", quote = F)
  remove(dat)
