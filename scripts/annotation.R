vcf_annotation <- function(vcf_file, gff_file, output_dir="snpsift") {
  library(vcfR)
  library(reshape2)
  library(txdbmaker)
  ...
  writeVcf(vcf_gr, "annotated_variants.vcf")
  ...
  tasObj <- readGenotypeTableFromPath("annotated_variants.vcf")
...
#   # Replace IUPAC codes with corresponding alleles
   ...
   hapdir <- file.path(getwd(), output_dir)
   ...
   generate_genefiles <- function(chrom){
     ...
     write.csv(locus, file = file.path(hapdir,paste0(gene_id, ".csv")))
     return(gene_id)
   }
...
# Example usage:
# vcf_annotation("fin_magic_geno.vcf", "Oryza_sativa.IRGSP-1.0.60.gff3",genome="Oryza_sativa")
